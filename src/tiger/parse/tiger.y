...
exp:
  /*
  //raw sequence according to the testX.tig
...
expseq:
    /*empty*/ {$$ = new A::SeqExp(errormsg.tokPos, nullptr);}
...
one:
  ...
  | ID DOT ID {$$ = new A::FieldVar(
      errormsg.tokPos, 
      new A::SimpleVar(errormsg.tokPos, $1), 
      $3
      );
    }
  | ID LBRACK exp RBRACK {$$ = new A::SubscriptVar(
      errormsg.tokPos, 
      new A::SimpleVar(errormsg.tokPos, $1), 
      $3
      );
...
tyfields_nonempty:
    ID COLON ID {$$ = new A::FieldList(
      new A::Field(errormsg.tokPos, $1, $3),
      nullptr
      );
    } 
  | ID COLON ID COMMA tyfields_nonempty {$$ = new A::FieldList(
      new A::Field(errormsg.tokPos, $1, $3),
      $5
      );
    }
...
fundec_one:
    FUNCTION ID LPAREN tyfields RPAREN EQ exp {$$ = new A::FunDec(errormsg.tokPos, $2, $4, nullptr, $7);}
